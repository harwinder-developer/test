...
$(document).ready(function (){
    ...
    // adjust fixed bar on mobile / admin bar. ONLY FIX MARCHE PSA
    var timeoutScroller = '';
    $(document).scroll(function() {
        if($('#wpadminbar').length 
            ...
            clearTimeout(timeoutScroller);
            timeoutScroller = setTimeout(function( ){
                    
                if($(document).scrollTop() < 46) {
                    $('#tpbr_topbar').css({ top : 46 - $(document).scrollTop() });
                } else {
                    if($('#tpbr_topbar').css( 'top' ) != 0){
    ...
    if (fixed == 'fixed'){
        var fixed_result = 'position:fixed; z-index:9999; width:100%; left:0px; top:0; ';
    } else {
        var fixed_result = '';
    }
    ...
    if (status == 'active') {
        if (fixed == 'fixed'){
            ...
            $('<div id="tpbr_topbar" style="position:relative; z-index:9999; ' + fixed_result + ' background:' + tbcolor + '; padding:4px 20px 3px;"><div id="tpbr_box" style="line-height:40px; text-align:center; width:100%; color:white; font-size:15px; font-family: Helvetica, Arial, sans-serif; font-weight:300;">' + message + btn_result + '</div></div>').prependTo('body');
        }
        updateHeights();
