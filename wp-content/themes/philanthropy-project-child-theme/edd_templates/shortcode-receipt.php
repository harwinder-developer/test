	...
	<div class="purchase-receipt-container">
		...
		<tbody>
			...
			<?php if ( filter_var( $edd_receipt_args['price'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
				...
				<?php 
				// $covered_fees = edd_get_payment_meta( $payment->ID, 'covered_fee', true );
				$charge_details = edd_get_payment_meta( $payment->ID, 'charge_details', true );
				$donor_covers_fee = $charge_details['donor-covers-fee'];
				if( $donor_covers_fee == "yes" ): ?>
				<tr>
					<td><strong><?php _e( 'Covered Fees', 'easy-digital-downloads' ); ?></strong></td>
					<td>
				...
				<tr>
					...
					<td>
						...
						if( $donor_covers_fee == "yes" ){
							$total_including_fees = floatval( edd_get_payment_amount( $payment->ID ) ) + floatval($charge_details['total-fee-amount']);
